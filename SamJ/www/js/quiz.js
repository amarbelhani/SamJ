  ...
  */
  $.each(questions, function(question_index, question) {
    ...
    $.each(question.answers, function(answer_index, answer) {
      ...
      if (last_question) {
        opts.confirmButtonText = "Voici vos résultats";
      }
  ...
  var $social = $("<div>")
    ...
    .html('<div id = "social-text">Vous avez aimé ce test ! Partagez le avec vos amis !</div> <br><br><a class="btn btn-primary" href="index.html"> < Retour</a>')
    .appendTo($results_slide);
...
function resultsText(state) {
  ...
  switch (true) {
    case (ratio === 1):
      text = "Wow&mdash;superbe score!";
      break;
    case (ratio > 0.9):
      text = "pas mal , Vous avez quasiment tout bon !";
      break;
    case (ratio > 0.60):
      text = "C'est pas mal, on va dire que c'est bon !";
      break;
    case (ratio > 0.5):
      text = "Bien, mais vous pourriez faire encore mieux !";
      break;
    case (ratio < 0.5 && ratio !== 0):
      text = " Dommage ! Vous devriez refaire le test !";
      break;
    case (ratio === 0):
      text = "Aucune réponse correcte ! Vous devriez réessayer";
      break;
  }
  return text;
